...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel for private(i,j,hash) shared(wl_size, num_hf, wl, bv_size) schedule(guided)	
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	misspelled = 0;
	#pragma omp parallel for private(j,hash) shared(bv_size, word) schedule(guided)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
