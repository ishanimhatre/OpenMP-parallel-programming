...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel for shared(wl_size, num_hf, wl, bv_size) private(i,j,hash) schedule(guided,500)
	// schedule(static, 100)	
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	misspelled = 0;
	#pragma omp parallel for shared(bv_size, word) private(j,hash) schedule(guided)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
